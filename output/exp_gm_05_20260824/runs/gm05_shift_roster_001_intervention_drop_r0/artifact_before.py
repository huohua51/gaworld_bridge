...
def assign(workers, shifts):
    ...
    """
    assignments = []
    for shift_id, (start_time, end_time, kind) in shifts.items():
        if kind == 'night' and len(assignments) < NIGHT_SHIFT_MINIMUM:
            continue
        for worker_id, (name, is_medic, certification) in workers.items():
            if is_medic and certification and valid_roster(assignments, workers, shifts):
