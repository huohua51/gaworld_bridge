#!/usr/bin/env python3
"""Run preregistered C1-04 fixture calibration or GLM-5.2 seed-0 matrix."""
...
from exp_gm_c1_04.loader import TASK_IDS, VARIANTS, load_tasks
from exp_gm_c1_04.protocol import run_cell
from exp_gm_c1_04.scorer import SCORER_VERSION, WORKFLOW_ID, score_cell
...
def _registration() -> tuple[dict[str, Any], str]:
    ...
    for relative, expected in (registration.get("frozen_inputs") or {}).items():
        path = (BRIDGE_ROOT / str(relative)).resolve()
        if not path.is_file():
    ...
    actual_gaworld_commit = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=str(GAWORLD_ROOT), text=True
    ).strip()
    if actual_gaworld_commit != str(registration.get("required_gaworld_commit") or ""):
        errors.append(f"gaworld_commit_mismatch:{actual_gaworld_commit}")
    if errors:
...
def _gate(cells: list[dict[str, Any]], coverage: float) -> str:
    ...
    if _rate(cells, "nack_path_coverage", variant="intervention") != 1.0:
        return "nack_path_missed"
    if _rate(cells, "nack_path_coverage", variant="control") != 0.0:
        return "control_spurious_nack"
    if _rate(cells, "retry_recovery_success", variant="intervention") != 1.0:
        return "retry_not_recovered"
    if _rate(cells, "platform_identifier_ownership") != 1.0:
        return "platform_identifier_ownership_failed"
    if _rate(cells, "stale_plan_rejected") != 1.0:
        return "stale_plan_accepted"
...
def run_matrix(
    ...
    *,
    ...
    if client.info.live and (
        client.info.provider != provider.get("name")
        or client.info.model_version != provider.get("model")
    ):
    ...
    for task in load_tasks():
        for variant in VARIANTS:
            run_id = f"c1_v4_{task['id']}_{variant}_full_s0"
            ...
            cell = score_cell(task=task, variant=variant, seed=0, loop=loop)
    ...
    first_errors = Counter((cell.get("process_profile") or {}).get("first_error") for cell in cells)
    report = {
        "experiment_id": "EXP-GM-C1-04",
        ...
        "nack_path_intervention": _rate(cells, "nack_path_coverage", variant="intervention"),
        "nack_path_control": _rate(cells, "nack_path_coverage", variant="control"),
        "retry_recovery_intervention": _rate(cells, "retry_recovery_success", variant="intervention"),
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=BRIDGE_ROOT / "output" / "exp_gm_c1_04_20260827")
    args = parser.parse_args()
    if args.max_calls != MAX_CALLS or args.max_tokens != MAX_TOKENS or args.temperature != TEMPERATURE:
    ...
    if args.fixture_oracle:
        ...
        if not args.allow_live_model or not args.provider:
        ...
        if args.provider != provider["name"]:
            parser.error(f"registered provider is {provider['name']}")
