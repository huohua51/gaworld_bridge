...
def test_v1_severity_7_high():
    assert is_high_priority({"severity": 7, "region": "north"}) is True


def test_v1_severity_6_not_high():
    assert is_high_priority({"severity": 6, "region": "north"}) is False


def test_closed_and_capacity():
    north = route({"severity": 9, "region": "north"}, CENTERS)
...
def test_uncovered_region():
    out = route({"severity": 9, "region": "east"}, CENTERS)
