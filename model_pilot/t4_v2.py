"""Model-driven T4-v2 registered-transport loop."""
...
def _prompt(
    *,
    ...
    return json.dumps(
        {
            "protocol": "gaworld-benchmark-t4-model-v2",
            "instruction": (
                "Follow the registered transport rules below. Return exactly one "
                "JSON object matching response_schema. Do not use markdown or add "
                "fields. Use only received evidence."
            ),
            "registered_transport_rules": [
                (
                    "source: when a valid registered message and "
                    "next_registered_node are present, forward must be true"
                ),
                (
                    "intermediate: when message_received is true, accept and "
                    "forward must both be true"
                ),
                (
                    "target: when message_received is true, accept must be true "
                    "and target_action must equal message.payload.target_action"
                ),
                (
                    "target: when message_received is false, accept must be false "
                    "and target_action must be keep_current"
                ),
                "action_required affects only target action, never transport",
            ],
            "task_family": "T4",
            "message_class": "registered_status_update",
            "role": role,
            "node_id": node_id,
...
def run_cell(
    ...
    if track == "remove_bridge":
        intervention = channel.remove_edge(
            *bridge, intervention_id=f"model-v2-remove-{task['id']}-bridge"
        )
    ...
    for index, (sender, receiver) in enumerate(_edges(route)):
        if not can_forward:
            break
