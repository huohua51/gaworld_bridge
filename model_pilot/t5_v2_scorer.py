"""Evidence-only scorer for the T5-v2 model pilot."""
...
from exp_gm_t5_02.scorer import expected_actions
...
WORKFLOW_ID = "model_pilot_t5_v2"
SCORER_VERSION = "model-pilot-t5-v2-scorer-v1"


def _semantic_decisions(model: dict[str, Any]) -> list[dict[str, Any]]:
    responses = {
    ...
    for request in model.get("requests") or []:
        ...
        state = (prompt.get("registered_policy_state") or {}).get("status")
        parsed = response.get("parsed") or {}
        expected_notice = state != "absence"
        expected_binding = state == "binding"
        correct = (
            response.get("ok") is True
            and parsed.get("notice_seen") is expected_notice
            and parsed.get("binding") is expected_binding
        )
        decisions.append(
            {
                "agent_id": str(request.get("agent_id") or ""),
                "policy_state": str(state or ""),
                "expected_notice_seen": expected_notice,
                "actual_notice_seen": parsed.get("notice_seen"),
                "expected_binding": expected_binding,
                "actual_binding": parsed.get("binding"),
                "correct": correct,
...
def score_cell(
    *,
    ...
    semantics = _semantic_decisions(model)
    semantics_correct = len(semantics) == len(task["residents"]) and all(
        item["correct"] for item in semantics
    )
    ...
    if not model["contract_ok"]:
        ...
        first_error = "policy_semantics_incorrect"
    ...
    run_id = f"model_v2_{task['id']}_{policy_state}_{track}_s{seed}"
    ...
    semantic_evidence = [
        str(item["evidence_id"]) for item in semantics if item["evidence_id"]
    ]
    context = RunContext(
        run_id=run_id,
        task_id=str(task["id"]),
        ...
        trace_version="urban-policy-and-model-jsonl-v2",
    ...
    cell = compose_cell(
        ...
        criteria=[
            CriterionResult(
                ...
                "R2",
                "registered_policy_state_v2",
                True,
                1.0 if semantics_correct else 0.0,
                passed=semantics_correct,
                critical=True,
                evidence_ids=semantic_evidence,
                detail=json.dumps(semantics, ensure_ascii=False, sort_keys=True),
            ),
            CriterionResult(
        ...
        process_profile={
            ...
            "events": event_names,
            "policy_semantic_decisions": semantics,
