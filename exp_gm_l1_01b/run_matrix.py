#!/usr/bin/env python3
"""EXP-GM-L1-01b. Full Multi interruption recovery. Direct is solvability only."""
...
from exp_gm_l1_01b.budget import KINDS, MODEL, PROVIDER, TEMPERATURE
from exp_gm_l1_01b.loader import TRACKS, VARIANTS, load_tasks
from exp_gm_l1_01b.loop import run_cell, run_direct
from exp_gm_l1_01b.prompts import rule_checkpoint, rule_direct, rule_handoff, rule_resume, rule_step
from exp_gm_l1_01b.rule_tests import main as rule_main
from exp_gm_l1_01b.scorer import score_cell
from exp_gm_l1_01b.loader import solve_outputs
...
WORKFLOW_ID = "exp_gm_l1_01b_handoff"
EXPERIMENT_ID = "EXP-GM-L1-01b"
OUT = BRIDGE_ROOT / "output" / "exp_gm_l1_01b_20260825"
...
def _load_freeze() -> dict:
    ...
    from exp_gm_l1_01b.freeze import write_manifest
...
def _seed0_gate(cells: list[dict], coverage: float) -> str:
    ...
    if not _budget_ok(cells) or not _isolation_ok(cells):
        return "A_r0"
    multi_p = _pattern(cells, "multi")
    drop_c = _pattern(cells, "drop_checkpoint")
    drop_h = _pattern(cells, "drop_handoff")
    if multi_p == "both_fail" and drop_c == "both_fail" and drop_h == "both_fail":
        return "C_floor"
    if multi_p == "both_pass" and drop_c == "both_pass" and drop_h == "both_pass":
        return "C_ceiling"
    if multi_p == "both_pass" and drop_c == "drop_works" and drop_h == "drop_works":
        return "construct_open"
    if multi_p == "both_pass" and (drop_c == "drop_works") != (drop_h == "drop_works"):
        return "single_drop"
    if multi_p == "both_fail":
        return "C_floor"
    return "off_floor"
...
def _interpret(gate: str, *, direct_ok: bool, full: dict[str, float | None], phase: str = "") -> str:
    ...
    if gate == "construct_open":
        return "Full Multi 通过，两种 Drop 的 intervention 失败。构念被测开。仍是 seed0，不自动补 54 格。"
    if gate == "single_drop":
...
def _claim(gate: str, interpretation: str, *, direct_ok: bool, phase: str = "") -> str:
    ...
    if gate == "construct_open":
        return "构念被测开。seed0 完成，不自动补 54 格。"
    if gate == "single_drop":
...
def _pack(
    ...
    *,
    ...
    payload = {
        ...
        "interruption_recovery_result": "N/A" if phase in {"rule", "direct"} else None,
        "repeat_1_2_allowed": gate == "construct_open",
        "c1_status": "development_partial_pass",
        "does_not_overwrite": ["EXP-GM-L1-01", "EXP-GM-C1-01", "EXP-GM-C1-02", "EXP-GM-C1-03"],
        ...
        "do_not": ["run_c1_04", "tune_c1_prompts", "create_c1_holdout", "run_multi_this_round"],
    }
    ...
    lines = [
        ...
        f"- 解释：{interpretation}",
        "",
        f"**结论：** {_claim(gate, interpretation, direct_ok=direct_ok, phase=phase)} 功能进度：Direct 未过或 seed0 测量无效则仍约 75%；seed0 Coverage=1.0 后约 85%。不做 C1-04。",
        "",
    ]
    if cells:
...
def main() -> int:
    ...
    if rule_main() != 0:
        return 1
    from exp_gm_l1_01b.fairness import preflight
