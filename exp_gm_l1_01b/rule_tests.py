#!/usr/bin/env python3
"""Rule calibration for EXP-GM-L1-01b. Must pass before GLM."""
...
from exp_gm_l1_01b.loader import BANNED, load_task, load_tasks, resume_step_for, solve_outputs, solve_step
...
def _run(tmp: Path, task: dict, variant: str, track: str, **overrides):
    ...
    cell = score_cell(
        ...
        repeat_id=0,
        loop=loop,
        workflow_id="exp_gm_l1_01b_rule",
...
def test_not_old_items():
    ...
    assert ids == {"l1_01_inventory_lots_001", "l1_01_centrifuge_rotor_001", "l1_01b_file_intake_001"}
...
def test_file_intake_copies_ids():
    task = load_task("l1_01b_file_intake_001")
    out = solve_outputs(task)
    blob = json.dumps(out)
    assert "checksum" not in blob
    assert out["file_ck41"] == {"verified_ids": ["D17", "D18"], "missing_ids": []}
    assert out["file_ar41"] == {"folder": "F2", "sealed": True}
    assert solve_step(task, "file_ck41", {"file_rx41": out["file_rx41"]}) == out["file_ck41"]
...
def main() -> int:
    ...
    test_file_intake_copies_ids()
    print("file intake copies received_ids, no checksum")
    test_direct_rule()
    ...
    test_deviations()
    print("duplicate / wrong resume / stale checkpoint caught")
    test_fairness()
    print("fairness pass")
    print("EXP-GM-L1-01b Rule calibration passed.")
    return 0
