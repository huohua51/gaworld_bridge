...
from exp_gm_l1_01.contract import CHECKPOINT_VERSION
from exp_gm_l1_01.loader import public_spec, resume_step_for, solve_outputs, solve_step, worker_private
...
def successor_step_prompt(
    ...
    verify_note = ""
    if step_id.endswith("verify_sp71") or step_id == "verify_sp71":
        verify_note = (
            "校验输出必须根据检查点中的采集结果计算："
            f"checksum 为 {private.get('verify_prefix') or 'vfy-'} 接标本号，count 为标本数量。\n"
        )
    if step_id == "archive_sp71":
        verify_note = "归档必须带上校验得到的 seal，并使用本步材料中的 bin。\n"
    return (
        _spec_block(task)
