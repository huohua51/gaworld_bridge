...
TASK_IDS = ("l1_01_inventory_lots_001", "l1_01_centrifuge_rotor_001", "l1_01_specimen_log_001")
...
def _verify_output(task: dict[str, Any], outputs: dict[str, Any]) -> dict[str, Any]:
    intake_id = task["step_ids"][0]
    intake = outputs.get(intake_id) or task["materials"][intake_id]
    specimens = list(intake["specimens"])
    prefix = str(task.get("verify_prefix") or "vfy-")
    return {"checksum": prefix + "-".join(specimens), "count": len(specimens)}


def _archive_output(task: dict[str, Any], outputs: dict[str, Any]) -> dict[str, Any]:
    verify_id = task["step_ids"][1]
    verify = outputs.get(verify_id) or _verify_output(task, outputs)
    archive = dict(task["materials"][task["step_ids"][2]])
    archive["seal"] = verify["checksum"]
    return archive
...
def solve_outputs(task: dict[str, Any]) -> dict[str, Any]:
    ...
    if task["id"] == "l1_01_specimen_log_001":
        outputs[step_ids[0]] = dict(task["materials"][step_ids[0]])
        outputs[step_ids[1]] = _verify_output(task, outputs)
        outputs[step_ids[2]] = _archive_output(task, outputs)
        return outputs
...
def solve_step(task: dict[str, Any], step_id: str, prior: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if step_id not in full:
        return {}
    if task["id"] == "l1_01_specimen_log_001" and step_id == task["step_ids"][1]:
        return _verify_output(task, outputs)
    if task["id"] == "l1_01_specimen_log_001" and step_id == task["step_ids"][2]:
        return _archive_output(task, outputs)
