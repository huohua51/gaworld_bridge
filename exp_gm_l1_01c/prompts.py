...
from exp_gm_l1_01c.contract import CHECKPOINT_VERSION
from exp_gm_l1_01c.loader import public_spec, resume_step_for, solve_outputs, solve_step, worker_private
...
def rule_handoff(task: dict[str, Any], variant: str, checkpoint: dict[str, Any] | None) -> dict[str, Any]:
    ...
    remaining = [sid for sid in task["step_ids"] if sid not in set(completed)]
...
def checkpoint_prompt(task: dict[str, Any], completed: list[str], outputs: dict[str, Any]) -> str:
    return (
        ...
        + "outputs 只能包含已完成步骤的结果，不要写入尚未完成的步骤。\n"
...
def coordinator_prompt(
    ...
    *,
    ...
    body = (
        _spec_block(task)
        + "你是 Coordinator。读取检查点，指定接替者与恢复步骤。不能执行具体步骤，不能给出剩余步骤的正确业务输出。\n"
        + "resume_step 必须是有序步骤中第一个不在 completed_steps 里的 id。不要根据 outputs 的键判断哪些步骤已完成。\n"
        + "不要重复已完成步骤，不要跳过中间步骤。remaining_steps 必须是全部尚未完成的步骤，按原顺序，且第一项等于 resume_step。\n"
        + '输出 {"successor": "worker_a|worker_b", "checkpoint_version": "ckpt-001", "completed_steps": [...], "resume_step": "<id>", "remaining_steps": [...]}。\n'
        + f"【有序步骤】{json.dumps(task['step_ids'], ensure_ascii=False)}\n"
        + f"【已送达检查点】{json.dumps(checkpoint, ensure_ascii=False)}\n"
    )
    if a_unavailable and variant == "intervention":
        body += f"【状态】{task['interrupt_token']}。Worker A 已不可用，必须指定 worker_b 接替。续做位置仍由 completed_steps 决定。\n"
    else:
        body += "【状态】Worker A 仍可继续。指定 worker_a 从第一个尚未完成的步骤继续。\n"
    return body
...
def successor_step_prompt(
    ...
    if task.get("kind") == "receive_verify_archive" and step_id == task["step_ids"][2]:
        verify_note = "入库使用本步材料中的字段，sealed 必须为 true。\n"
    return (
        _spec_block(task)
