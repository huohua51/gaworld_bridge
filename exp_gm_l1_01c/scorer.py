...
from exp_gm_l1_01c.contract import CHECKPOINT_VERSION, EXTRACTABLE_STEP_ERRORS
from exp_gm_l1_01c.loader import oracle_plan, solve_outputs
...
def _checkpoint_content_correct(task: dict[str, Any], world: dict[str, Any], want_out: dict[str, Any]) -> bool:
    ck = world.get("checkpoint") or {}
    first = task["step_ids"][0]
    completed = [str(item) for item in (ck.get("completed_steps") or [])]
    outputs_ck = dict(ck.get("outputs") or {})
    return completed == [first] and set(outputs_ck.keys()) == {first} and outputs_ck.get(first) == want_out.get(first)
...
def first_error(*, track: str, variant: str, loop: dict[str, Any], want: dict[str, Any], task: dict[str, Any]) -> str:
    ...
    if not _checkpoint_content_correct(task, world, want_out):
        return "checkpoint_content_incorrect"
    ...
    coord_resume = str((loop.get("handoff") or {}).get("resume_step") or "")
    if track != "direct" and coord_resume and coord_resume != expected_resume:
        return "resume_from_wrong_step"
    if track != "direct" and declared and declared != expected_resume:
        return "resume_from_wrong_step"
...
def score_cell(
    *,
    ...
    content_ok = True if track == "direct" else _checkpoint_content_correct(task, world, want_out)
    coord_resume = str((loop.get("handoff") or {}).get("resume_step") or "")
    expected_resume = str(want.get("resume_step") or task["step_ids"][1])
    resume_step_correct = True if track == "direct" else (coord_resume == expected_resume and resume_ok)
    remaining = list((loop.get("handoff") or {}).get("remaining_steps") or [])
    remaining_ok = True if track == "direct" else remaining == list(task["step_ids"][1:])
    not_repeated = True if track == "direct" else expected_resume not in set((loop.get("handoff") or {}).get("completed_steps") or [])
    adopted = False if track == "direct" else bool(handoff_ok and resume_ok and str(loop.get("resume_declared") or "") == expected_resume)
    skipped = 0.0 if remaining_ok else 1.0
    extractable = True
    if track == "direct":
    ...
    if expect_success and track != "direct":
        conditioned = bool(conditioned and resume_ok and handoff_ok and delivered and content_ok and resume_step_correct and remaining_ok and not_repeated)
    if track == "drop_checkpoint" and variant == "intervention":
        conditioned = False
    if track == "drop_handoff" and variant == "intervention":
        conditioned = False
    ...
    cell = compose(
        ...
        criteria=[
            ...
            CriterionResult("checkpoint_delivered_to_successor", "R2", "checkpoint", True, 1.0 if delivered else 0.0, passed=delivered),
            CriterionResult("checkpoint_content_correct", "R2", "checkpoint", True, 1.0 if content_ok else 0.0, passed=content_ok),
            CriterionResult("handoff_delivered", "R2", "handoff", True, 1.0 if handoff_ok else 0.0, passed=handoff_ok),
            CriterionResult("handoff_adopted", "R2", "handoff", True, 1.0 if adopted else 0.0, passed=adopted),
            CriterionResult("resume_step_correct", "R2", "resume", True, 1.0 if resume_step_correct else 0.0, passed=resume_step_correct),
            CriterionResult("completed_step_not_repeated", "R2", "resume", True, 1.0 if not_repeated else 0.0, passed=not_repeated),
            CriterionResult("remaining_step_not_skipped", "R2", "resume", True, 1.0 if remaining_ok else 0.0, passed=remaining_ok),
            CriterionResult("target_correct", "R2", "world", True, 1.0 if complete else 0.0, passed=complete),
        ...
        extra={
            ...
            "checkpoint_delivered_to_successor": delivered,
            "checkpoint_content_correct": content_ok,
            "handoff_delivered": handoff_ok,
            "handoff_adopted": adopted,
            "resume_step_correct": resume_step_correct,
            "completed_step_not_repeated": not_repeated,
            "remaining_step_not_skipped": remaining_ok,
            "target_correct": complete,
            ...
            "missing_remaining_step_rate": skipped,
            ...
            "env_denied": bool(loop.get("env_denied")),
            "first_error": err,
            "got_outputs": outputs,
            "want_outputs": want_out,
            "got_actors": actors,
            "want_actors": want.get("actor_by_step"),
            "ranking_note": "L1-01c interruption recovery regression, not a leaderboard",
        },
    )
