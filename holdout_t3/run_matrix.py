#!/usr/bin/env python3
"""HO-GM-T3-01 discrimination. Direct doability, then seed0. Repeats only if off floor and ceiling."""
...
from holdout_t3.freeze import write_manifest
from holdout_t3.loader import TRACKS, VARIANTS, load_tasks
from holdout_t3.loop import generate_shared_draft, run_track_from_draft
from holdout_t3.prompts import reviewer_prompt, rule_builder_draft, rule_executor, rule_review, strip_source
from holdout_t3.rule_tests import main as rule_main
from holdout_t3.scorer import score_cell
...
WORKFLOW_ID = "holdout_t3_discrimination"
        ...
        for variant in VARIANTS:
            ...
            for track in tracks:
                ...
                def revise_fn(prompt: str, review, t=task, lg=log):
                    from holdout_t3.rule_tests import _draft_from_prompt
...
def main() -> int:
    ...
    if rule_main() != 0:
        return 1
    from holdout_t3.fairness import preflight
    import yaml

    out = BRIDGE_ROOT / "output" / "holdout_t3_20260825"
    ...
    if args.phase == "rule":
        print("Rule 已过门并冻结。下一步 Direct，再用 --phase seed0。不补 repeat。", flush=True)
        return 0
    ...
    if gate in {"C_floor", "C_ceiling"}:
        ...
        return 0
    print("gate=off_floor。留出只跑 seed0 一次，不补 repeat 1/2。", flush=True)
    return 0
