"""T3-03 scoring. Hidden tests and file values, not model claims."""
...
from holdout_t3.fairness import call_kinds_ok, oracle_in_prompt, v2_leaks_in_prompt
from holdout_t3.inspect import apply_status, decision_correct, evidence_correct
from holdout_t3.prompts import required_value
...
def first_error(
    *,
    ...
    if track in {"single", "multi", "direct"} and extractable and decision_ok and not read_ok and not (track == "drop"):
...
def score_cell(
    *,
    ...
    integrity_track = "single" if track == "direct" else track
    if integrity_track == "drop":
    ...
    review_leaks = v2_leaks_in_prompt(task, str(loop.get("review_prompt") or ""))
    ...
    single_private_leak = bool(review_leaks) if track == "single" else False
    ...
    cell = compose(
        ...
        measurement_gates=[
            ...
            GateResult("single_no_private_v2", not single_private_leak, layer="R0"),
        ...
        extra={
            ...
            "first_error": err,
            "got": got,
            "ranking_note": "discrimination experiment, not a leaderboard",
        },
    )
