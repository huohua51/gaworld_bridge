#!/usr/bin/env python3
"""EXP-GM-I1: TASK-I1 Verified Information Relay. Seed 0 first."""
...
from exp_i1.loop import run_cell_loop
from exp_i1.probes import PROBES, variant_spec
from exp_i1.roles import (
    dispatcher_prompt,
    observer_prompt,
    parse_json_object,
    rule_dispatcher,
...
from exp_i1.scoring import first_error, process_success, r0_ok
...
WORKFLOW_ID = "exp_i1_verified_information_relay"
...
def main() -> int:
    ...
    _pin_glm()
    out = BRIDGE_ROOT / "output" / "exp_i1_20260824"
    out.mkdir(parents=True, exist_ok=True)
    if len(sys.argv) > 1 and sys.argv[1] == "focused-rerun":
        prev = out / "REPORT.md"
        if prev.exists() and not (out / "REPORT_before_focused_contract.md").exists():
            (out / "REPORT_before_focused_contract.md").write_text(prev.read_text(encoding="utf-8"), encoding="utf-8")
        kept = load_cells_except_track(out, "focused")
        print(f"phase=focused-contract-rerun kept={len(kept)}", flush=True)
        focused = run_focused_matrix(out, [0, 1, 2], mode="llm")
        payload = _pack(kept + focused, out, phase="focused_contract_rerun")
        print((out / "REPORT.md").read_text(encoding="utf-8"))
        return 0 if payload["summary"]["measurement_valid"] >= 72 else 1
