...
from exp_i1.probes import PROBES
from exp_i1.roles import parse_json_object, rule_dispatcher, rule_observer, rule_verifier
from exp_i1.scoring import process_success
...
def test_action_contract_rejects_swap():
    from exp_i1.roles import action_contract

    probe = PROBES[1]
    payload, reason = action_contract(
        probe, {"action": "send_to_A", "value": "submit_shelter", "adopted_state_version": "v1", "evidence_message_id": "x"}
    )
    assert reason == "swapped_action_value"
    ok, reason_ok = action_contract(
        probe, {"action": "submit_shelter", "value": "send_to_A", "adopted_state_version": "v1", "evidence_message_id": "x"}
    )
    assert reason_ok == "ok"
