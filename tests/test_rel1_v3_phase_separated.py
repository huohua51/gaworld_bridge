...
from exp_rel1_03.loader import VARIANTS, load_tasks
from exp_rel1_03.protocol import run_cell
from exp_rel1_03.scorer import score_cell


def test_rel1_v3_fixture_passes_phase_separated_matrix(tmp_path: Path) -> None:
    ...
    assert all(cell["extra"]["observer_relay_faithful"] for cell in cells)
    assert all(cell["extra"]["formation_counts_correct"] for cell in cells)
    assert all(cell["extra"]["formation_source_correct"] for cell in cells)
    assert all(cell["extra"]["update_source_correct"] for cell in cells)
