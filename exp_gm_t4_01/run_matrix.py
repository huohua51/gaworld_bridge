"""Run the deterministic T4-01 platform and scorer calibration matrix."""
...
from exp_gm_t4_01.loader import TRACKS, VARIANTS, load_tasks
from exp_gm_t4_01.loop import run_cell
from exp_gm_t4_01.scorer import score_cell
...
def _valid(
    ...
    return [
        ...
        and (cell.get("extra") or {}).get("run_context", {}).get("track") == track
        and (
            variant is None
            or (cell.get("extra") or {}).get("run_context", {}).get("variant")
            == variant
        )
    ]
...
def _strict_pair(cells: list[dict[str, Any]], track: str) -> float | None:
    ...
    passed = sum(
        1
        for group in pairs
        if group["control"].get("full_pass") == 1
...
def summarize(cells: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if (
        ...
        and by_variant["remove_bridge"] == {"control": 1.0, "intervention": 0.0}
        and by_variant["drop_bridge"] == {"control": 1.0, "intervention": 0.0}
    ):
        gate = "rule_calibration_pass"
    return {
        "experiment_id": "EXP-GM-T4-01",
        ...
        "CommunicationValueRemoveBridge": communication_value_remove,
        "CommunicationValueDropBridge": communication_value_drop,
        "claim": "平台多跳通道与正负控校准通过；尚未运行模型。"
    ...
    (out / "REPORT.md").write_text(
        "\n".join(
            [
                ...
                "- ranking_eligible: false",
                "- 结论：仅证明平台多跳传播、桥边干预和Scorer正负控可用，尚未调用模型。",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def main() -> int:
    ...
    parser.add_argument(
        "--out", type=Path, default=BRIDGE_ROOT / "output" / "exp_gm_t4_01_v1"
    )
