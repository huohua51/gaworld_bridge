#!/usr/bin/env python3
"""Run the frozen REL1-v3 fixture or GLM-5.2 seed-0 matrix."""
...
from exp_rel1_03.fixture import fixture_client
from exp_rel1_03.loader import TASK_IDS, VARIANTS, load_tasks
from exp_rel1_03.protocol import run_cell
from exp_rel1_03.scorer import SCORER_VERSION, WORKFLOW_ID, score_cell
...
def run_matrix(
    ...
    for task in load_tasks():
        for variant in VARIANTS:
            run_id = f"rel1_v3_{task['id']}_{variant}_full_s0"
    ...
    report = {
        ...
        **{field: _rate(cells, field) for field in metric_fields},
        "first_error": dict(
            Counter((cell.get("process_profile") or {}).get("first_error") for cell in cells)
        ),
        ...
        "does_not_overwrite": ["EXP-GM-REL1", "EXP-GM-REL1-02"],
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=BRIDGE_ROOT / "output" / "exp_gm_rel1_03_20260827",
    )
    args = parser.parse_args()
    if (args.max_calls, args.max_tokens, args.temperature) != (
        MAX_CALLS,
        MAX_TOKENS,
        TEMPERATURE,
    ):
    ...
    if args.fixture_oracle:
        ...
        client = GAWorldModelClient(
            args.provider, temperature=TEMPERATURE, max_tokens=MAX_TOKENS
        )
