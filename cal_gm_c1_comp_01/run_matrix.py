#!/usr/bin/env python3
"""CAL-GM-C1-COMP-01: decouple conflict detection from reallocation. Not C1-01 tasks."""
...
from cal_gm_c1_comp_01.freeze import write_manifest
from cal_gm_c1_comp_01.loader import VARIANTS, load_tasks
from cal_gm_c1_comp_01.loop import run_cell
from cal_gm_c1_comp_01.prompts import agent_prompt, rule_agent
from cal_gm_c1_comp_01.rule_tests import main as rule_main
...
WORKFLOW_ID = "cal_gm_c1_comp_01"
PROVIDER = "paratera_glm"
MODEL = "GLM-4-Flash"
TEMPERATURE = 0
EXPERIMENT_ID = "CAL-GM-C1-COMP-01"
...
def _claim(summary: dict, *, phase: str) -> str:
    if summary["rates"]["Coverage"] < 1.0:
        return "Coverage 未过，回到 R0。不解释组件能力。"
    if summary["pass_gate"]["holds"]:
        if phase == "seed0":
            return "seed0 三组件都过。补 repeat 1/2 后冻结组件结论。"
        return summary["interpretation"]
    return summary["interpretation"] + " 不改 C1-01，不开 L1。"
...
def _pack(cells: list[dict], out: Path, *, phase: str, freeze: dict | None) -> dict:
    ...
    lines = [
        ...
        "| 组件 | FullPass | 含义 |",
        "| --- | ---: | --- |",
        f"| A 初始冲突检测 | {rates['A_initial_conflict']} | 调整前的初始方案是否冲突 |",
        f"| B 最终方案核验 | {rates['B_final_conflict_free']} | 最终分配是否无重复占用 |",
        f"| C 冲突后重新分配 | {rates['C_reallocate']} | 只交差，不交冲突布尔 |",
...
def main() -> int:
    ...
    if args.phase == "rule":
        ...
        return 0
    out = BRIDGE_ROOT / "output" / "cal_gm_c1_comp_01_20260825"
