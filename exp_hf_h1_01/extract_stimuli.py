...
def extract() -> dict:
    ...
    registry = []
    agent_dir = OUT / "stimuli" / "agent"
    display_dir = OUT / "stimuli" / "display"
    human_dir = OUT / "stimuli" / "human"
    ...
    for construct, task, variant in cells:
        ...
        (display_dir / f"{sid}.json").write_text(json.dumps(rater_view(trace), ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        human_slot = {
        ...
        if not slot_path.is_file():
            slot_path.write_text(json.dumps(human_slot, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        registry.append(
            {
                "stimulus_id": sid,
                "construct": construct,
    ...
    (OUT / "STIMULUS_REGISTRY.yaml").write_text(yaml.safe_dump(payload, allow_unicode=True, sort_keys=False), encoding="utf-8")
    (display_dir / "index.json").write_text(json.dumps({"stimuli": [c["stimulus_id"] for c in registry]}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return payload
