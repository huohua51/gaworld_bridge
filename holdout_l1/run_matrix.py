#!/usr/bin/env python3
"""HO-GM-L1-01. Full Multi interruption recovery. Direct is solvability only."""
...
from holdout_l1.budget import KINDS, MODEL, PROVIDER, TEMPERATURE
from holdout_l1.loader import TRACKS, VARIANTS, load_tasks
from holdout_l1.loop import run_cell, run_direct
from holdout_l1.prompts import rule_checkpoint, rule_direct, rule_handoff, rule_resume, rule_step
from holdout_l1.rule_tests import main as rule_main
from holdout_l1.scorer import score_cell
from holdout_l1.loader import solve_outputs
...
WORKFLOW_ID = "holdout_l1_handoff"
EXPERIMENT_ID = "HO-GM-L1-01"
OUT = BRIDGE_ROOT / "output" / "holdout_l1_20260825"
...
def _load_freeze() -> dict:
    ...
    from holdout_l1.freeze import write_manifest
...
def _interpret(gate: str, *, direct_ok: bool, full: dict[str, float | None], phase: str = "") -> str:
    ...
    if gate == "C_ceiling":
        return "三轨都高：Drop 无效或任务不依赖检查点/接替交付。留出只记 seed0。"
    if gate == "regression_pass":
        return "留出 seed0 通过预注册门。不补 repeat。不能写泛化或排名。"
    if gate == "development_regression_pass":
        return "开发集 54 格回归不属于本留出实验。"
    if gate == "repeats_off_pattern":
        return "留出禁止补 repeat。此门不应出现。"
    if gate == "single_drop":
        return "只识别出一种 Drop 机制的因果价值。分别报告，不能笼统说中断恢复通过。"
    multi = full.get("multi")
    if multi == 1.0:
        return "R0 有效且 Full Multi 通过。这是留出 seed0，不能扩写成一般性长期连续性已通过。"
...
def _claim(gate: str, interpretation: str, *, direct_ok: bool, phase: str = "") -> str:
    ...
    if gate == "C_ceiling":
        return "三轨都高：Drop 无效或任务不依赖交付。留出只记 seed0。"
    if gate == "regression_pass":
        return "留出 seed0 通过预注册门。不补 repeat。不能写泛化或排名。"
    if gate == "development_regression_pass":
        return "开发集 54 格回归不属于本留出实验。"
    if gate == "repeats_off_pattern":
        return "留出禁止补 repeat。此门不应出现。"
    if gate == "single_drop":
        return "只识别出一种机制。分别报告，不能笼统说中断恢复通过。"
...
def _pack(
    ...
    *,
    ...
    closed = gate == "regression_pass"
    payload = {
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "experiment_id": EXPERIMENT_ID,
        "parent": "EXP-GM-L1-01c",
        "phase": phase,
        "gate": gate,
        ...
        "checkpoint_causal_dependency": "observed_holdout_seed0" if closed else None,
        "handoff_causal_dependency": "observed_holdout_seed0" if closed else None,
        "interruption_recovery_holdout_result": (
            "N/A" if phase in {"rule", "direct"} else ("pass" if closed else "not_passed")
        ),
        "interruption_recovery_development_result": "see EXP-GM-L1-01c",
        "resume_workflow_regression": "see CAL-GM-L1-RESUME-01",
        "original_failure_status": "see EXP-GM-L1-01b",
        "interruption_recovery_result": "N/A" if phase in {"rule", "direct"} else ("holdout_seed0_pass" if closed else "holdout_seed0_not_passed"),
        "repeat_1_2_allowed": False,
        "repeat_1_2_run": False,
        ...
        "do_not": [
            ...
            "enter_ranking",
            "tune_protocol_and_rerun_same_holdout",
            "run_repeats",
        ],
    }
...
def _run_repeats(freeze: dict) -> int:
    print("留出禁止 --phase repeats。只跑 Direct + seed0 一次。", flush=True)
    return 1
...
def main() -> int:
    ...
    if rule_main() != 0:
        return 1
    from holdout_l1.fairness import preflight
    ...
    if args.phase == "rule":
        ...
        print("Rule 已过门并冻结。下一步 Direct 6 格，再 --phase seed0。不补 repeat。", flush=True)
        return 0
    ...
    if gate == "C_floor":
        ...
        return 0
    if gate == "regression_pass":
        print("gate=regression_pass。留出 seed0 完成。不补 repeat。不能写泛化或排名。", flush=True)
        return 0
    print(f"gate={gate}。留出 seed0 完成。不补 repeat。不得回头调协议。", flush=True)
    return 0
