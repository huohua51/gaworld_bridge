#!/usr/bin/env python3
"""Rule calibration for HO-GM-L1-01. Must pass before GLM."""
...
from holdout_l1.loader import BANNED, load_task, load_tasks, resume_step_for, solve_outputs, solve_step
...
def test_not_old_items():
    ...
    assert ids == {"l1_ho_crane_load_001", "l1_ho_fridge_log_001", "l1_ho_mail_bay_001"}
...
def test_label_intake_copies_ids():
    task = load_task("l1_ho_mail_bay_001")
    out = solve_outputs(task)
    blob = json.dumps(out)
    assert "checksum" not in blob
    assert out["mail_ck62"] == {"verified_ids": ["P3", "P4"], "missing_ids": []}
    assert out["mail_st63"] == {"bin": "B2", "sealed": True}
    assert solve_step(task, "mail_ck62", {"mail_rx61": out["mail_rx61"]}) == out["mail_ck62"]
...
def main() -> int:
    ...
    test_fairness()
    print("fairness pass")
    print("HO-GM-L1-01 Rule calibration passed.")
    return 0
