"""CHANGE-01 reviewer text + APPLY-01 executor text, filled with T3-01 fields."""
...
def rule_review(task: dict[str, Any], variant: str, draft: str) -> dict[str, Any]:
...
def reviewer_prompt(task: dict[str, Any], variant: str, draft: str, *, self_check: bool) -> str:
    observed = observed_value(task, draft)
    required = required_value(task, variant)
    keep_ex = {
        "decision": KEEP,
    ...
    role = (
        "你正在自检刚才写的代码。只能根据当前验收要求和草稿做决定。这一步不要改文件。\n"
        if self_check
        else "你是代码审核员。只能根据登记标准和草稿做决定。不要改文件。\n"
    )
    return (
